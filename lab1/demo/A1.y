...
char* macroArg1[20];
int numMacros1 = 0;
void defMacro1(char*,char*,char*);
int valMacro1(char*);
char* replaceArg1(char*,int);
%}
...
Statement : LCURLY Statements RCURLY {
    ...
    | ID LROUND RROUND SEMI {
		...
		int len = strlen(macroVal[macroNum]) + strlen($2) + strlen($3) + strlen($4) + 1;
		$$ = (char*) malloc((len + 1) * sizeof(char));
		$$[0] = '\0';
		strcat($$,$2);
...
Expression : PrimaryExpression OP PrimaryExpression {
    ...
    | ID LROUND Expression Expressions RROUND {
		int macroNum = valMacro1($1);
		char* mystring = replaceArg1($3, macroNum);
		int len = strlen(mystring) + strlen($2) + strlen($5);
		$$ = (char*) malloc((len + 1) * sizeof(char));
		$$[0] = '\0';
		strcat($$,$2);
		strcat($$,mystring);
...
void defMacro1(char* id, char* val, char* arg){
	macroID1[numMacros1] = (char*)malloc(strlen(id)+1);
	strcpy(macroID1[numMacros1], id);
	macroVal1[numMacros1] = (char*)malloc(strlen(val)+1);
	strcpy(macroVal1[numMacros1], val);
	macroArg1[numMacros1] = (char*)malloc(strlen(arg)+1);
	strcpy(macroArg1[numMacros1], arg);
	numMacros1++;
}

int valMacro1(char* id){
	for(int i = 0; i < numMacros; i++) {
		if(strcmp(macroID[i],id) == 0){		
			return i;
		}
...
char* replaceArg1(char* arg, int id){
	char* begin = macroVal1[id];
	char* match = strstr(begin, macroArg1[id]);
	char* mystring = "";
	while(match != NULL){
		strncat(mystring, begin, (int)match-(int)begin);
		strcat(mystring, arg);
		begin += strlen(macroArg1[id]);
		if(begin > macroVal1[id] + strlen(macroVal1[id])) {
			break;
		}
		match = strstr(begin, macroArg1[id]);
	}
